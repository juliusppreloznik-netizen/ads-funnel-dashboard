    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        <div className="flex flex-col items-start">
          <span className="text-xs font-medium text-vui-text">{getDisplayLabel()}</span>
          <span className="text-sm font-semibold text-vui-text-white">{formatDateRange()}</span>
        </div>

        {/* Chevron */}
        <svg
          className={`w-4 h-4 text-vui-text transition-transform ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute right-0 mt-2 bg-vui-page rounded-vui-xl shadow-2xl border border-vui-border/30 z-[9999]">
          <div className="flex">
            {/* Left Sidebar - Quick Selects */}
            <div className="w-44 border-r border-vui-border/30 py-3 bg-vui-body rounded-l-xl">
              <div className="px-4 py-2 text-xs font-semibold text-vui-text uppercase tracking-wider">
                Quick Select
              </div>
              <div className="space-y-0.5 px-2">
                {quickSelectOptions.map((option) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm rounded-vui transition-colors ${
            ...
            <div className="p-4">
              <div className="text-sm font-medium text-vui-text mb-4">
  ...
  return (
    <div className="w-64">
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={prevMonth}
          className="p-1 hover:bg-vui-sidenav-btn rounded-md transition-colors"
          type="button"
        >
          <svg className="w-5 h-5 text-vui-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span className="text-sm font-semibold text-vui-text-white">
      ...
      <div className="grid grid-cols-7 mb-2">
        {daysOfWeek.map((day) => (
          <div key={day} className="h-8 flex items-center justify-center text-xs font-medium text-vui-text">
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={`
                ...
                ${!disabled && !selected && !inRange ? 'hover:bg-vui-sidenav-btn' : ''}
                ${selected ? 'bg-blue-600 text-vui-text-white font-medium' : ''}
                ...
                ${!selected && !inRange && !todayDate && !disabled ? 'text-vui-text-white' : ''}
              `}
            >
              {date.getDate()}
            </button>
