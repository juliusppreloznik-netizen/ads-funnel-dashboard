  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-[#0f1535] border border-[rgba(226,232,240,0.3)] rounded-[15px] shadow-xl shadow-black/30 hover:bg-[#1a1f37] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
      >
        {/* Calendar Icon */}
        ...
        <div className="flex flex-col items-start">
          <span className="text-xs font-medium text-[#a0aec0]">{getDisplayLabel()}</span>
          <span className="text-sm font-semibold text-white">{formatDateRange()}</span>
        </div>

        {/* Chevron */}
        <svg
          className={`w-4 h-4 text-[#a0aec0] transition-transform ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 bg-[#0f1535] rounded-[20px] shadow-2xl border border-[rgba(226,232,240,0.3)] z-[9999]">
          <div className="flex">
            ...
            <div className="w-44 border-r border-[rgba(226,232,240,0.3)] py-3 bg-[#060b28] rounded-l-xl">
              <div className="px-4 py-2 text-xs font-semibold text-[#a0aec0] uppercase tracking-wider">
                Quick Select
              </div>
  ...
  return (
    <div className="w-64">
      ...
      <div className="grid grid-cols-7 mb-2">
        {daysOfWeek.map((day) => (
          <div key={day} className="h-8 flex items-center justify-center text-xs font-medium text-[#a0aec0]">
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={`
                ...
                ${inRange ? 'bg-blue-500/20 text-white' : ''}
                ${todayDate && !selected ? 'bg-orange-500/30 text-orange-400 font-semibold rounded-md' : ''}
